...
使用 WeRead 移动端扫码登录获取的 vid + JWT Token 鉴权（无 IP 绑定）。

═══════════════════════════════════════════════════════════
首次使用：扫码登录获取 Token（一次性操作，有效期约 10 年）
═══════════════════════════════════════════════════════════

1. 在 wewe-rss 界面或配套登录页扫码登录微信读书
...
【安全提示】Token 只存 GitHub Secrets，绝不放代码或日志
【有效期】Token 约 10 年，基本不需要续期
═══════════════════════════════════════════════════════════
"""
...
def get_json(url: str, headers: dict, retries: int = 2, backoff: float = 2.0) -> list | dict:
    ...
    for attempt in range(retries + 1):
        try:
            ...
            if resp.status_code >= 500:
                ...
                log.warning("HTTP %d，%.0fs 后重试…", resp.status_code, wait)
                time.sleep(wait)
                continue
...
def fetch_articles_for_mp(mp: dict, headers: dict) -> list[dict]:
    ...
    if not isinstance(data, list):
        log.warning("  ⚠️  [%s] 响应格式异常", mp_name)
        return []
...
# ═══════════════════════════════════════════════════════════
# C. 合并到前端统一文章库 data/articles.json
# ═══════════════════════════════════════════════════════════
...
def run() -> None:
    ...
    for acc, arts in sorted(by_account.items()):
        lines.append(f"## {acc}\n")
        for a in arts:
            lines.append(f"- [{a['title']}]({a['url'] or '#'})")
        lines.append("")
    ...
    log.info("✅ 完成！共 %d 篇文章，今日新增 %d 篇", len(all_articles), len(today_articles))
